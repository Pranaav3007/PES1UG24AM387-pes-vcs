...
static int write_tree_level(const Index *index, const char *prefix, ObjectID *tree_id_out) {
    Tree tree = {0};
    ...
    for (int i = 0; i < index->count; i++) {
        ...
        const char *remainder = entry->path + prefix_len;
        ...
        if (!slash) {
            ...
            TreeEntry *dst = &tree.entries[tree.count++];
            dst->mode = entry->mode;
            dst->hash = entry->hash;
            snprintf(dst->name, sizeof(dst->name), "%s", remainder);
            continue;
        }
        ...
        dst->mode = MODE_DIR;
        dst->hash = subtree_id;
        snprintf(dst->name, sizeof(dst->name), "%s", dir_name);
    }
